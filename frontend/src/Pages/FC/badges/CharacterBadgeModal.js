...
const BadgeModal = ({ character, isOpen, setOpen }) => {
    ...
    useEffect(() => {
        errorToaster(toastContext, apiCall(`/api/pilot/info?character_id=${character.id}`, {})
        .then((res) => {
    ...
    return <Modal open={isOpen} setOpen={setOpen}>
        <Box>
                ...
                <div style={{ marginBottom: '15px' }}>
                {
                    badges?.map((badge, key) => {
                        return (
                            <span key={key}>
                                { key % 3 ? null : <br /> }
                                <label style={{marginRight: '20px', marginBottom: '15px', display: 'inline-block'}} htmlFor={key}>
            ...
            <Button variant='success' onClick={onSubmit} disabled={pending}>
                <FontAwesomeIcon fixedWidth icon={!pending ? faCheck : faSpinner} spin={pending} style={{ marginRight: '10px'}} />
                Save Changes
            </Button>    
        </Box>
    </Modal>
